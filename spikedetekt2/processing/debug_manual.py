 ...
 #      pp = PdfPages('/home/skadir/alignment/multipagegraphs.pdf')
        if (interestpoint - chunk_size_less) <= s_start < (interestpoint):
           ...
           # plt.suptitle('Time %s ms'%(interestpoint_ms), fontsize=14, fontweight='bold')
            ...
            imdat = dataxis.imshow(np.transpose(chunk_raw[sampmin:sampmax,:][:,probe.channels]),interpolation="nearest",aspect="auto")
            plt.colorbar(imdat)
            ...
            imfil = filaxis.imshow(np.transpose(chunk_fil[sampmin:sampmax,:][:,probe.channels]),interpolation="nearest",aspect="auto")
            ...
            imwhite = whiteaxis.imshow(np.transpose(chunk_white[sampmin:sampmax,:][:,probe.channels]),interpolation="nearest",aspect="auto")
            #filaxis.set_xlabel('Samples')
            ...
            imwhiteraw = rawwhiteaxis.imshow(np.transpose(chunk_white_raw[sampmin:sampmax,:][:,probe.channels]),interpolation="nearest",aspect="auto")
            #filaxis.set_xlabel('Samples')
            ...
            imcomp = compaxis.imshow(np.transpose(chunk_threshold.weak[sampmin:sampmax,:][:,probe.channels].astype(int)+chunk_threshold.strong[sampmin:sampmax,:][:,probe.channels].astype(int)),interpolation="nearest",aspect="auto")
            ...
            imcon = conaxis.imshow(np.transpose(connected_comp_enum[sampmin:sampmax,:][:,probe.channels]),interpolation="nearest",aspect="auto");#plt.colorbar(imcon);
            plt.colorbar(imcon)
